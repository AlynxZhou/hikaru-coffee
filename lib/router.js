...
(function() {
  var Promise, Router, Site, URL, chokidar, colors, fse, getAbsPathFn, getContentType, getUrlFn, glob, http, isCurrentPathFn, moment, parseFrontMatter, path, yaml;
  ...
  Router = class Router {
          ...
          return this.themeWatcher.on(event, async(srcPath) => {
            ...
            if (event !== "unlink") {
              ...
              if (file["type"] === "template") {
                this.site.set("templates", (await Promise.all(this.site.get("templates").map((file) => {
    ...
    listen(ip, port) {
      ...
      process.prependListener("exit", () => {
        server.close();
        this.logger.log(`Hikaru stopped listening on http://${ip}:${port}${this.getAbsPath()}...`);
