...
(function() {
  var Generator, Hikaru, Logger, Processer, Promise, Renderer, Router, Translator, URL, cheerio, coffee, colors, dateStrCompare, escapeHTML, fse, getAbsPathFn, getUrlFn, glob, highlight, marked, moment, nib, nunjucks, paginate, paginateCategories, path, removeControlChars, sortCategories, stylus, yaml;
  ...
  module.exports = Hikaru = class Hikaru {
    ...
    registerInternalRenderers() {
      var coffeeConfig, headerIds, markedConfig, njkConfig, njkEnv, renderer, stylConfig;
      ...
      coffeeConfig = this.site["siteConfig"]["coffee"] || {};
    ...
    registerInternalProcessers() {
      this.processer.register("index", (p, posts, ctx) => {
        return new Promise((resolve, reject) => {
          ...
          try {
            ...
            return resolve(paginate(p, posts, ctx, this.site["siteConfig"]["perPage"]));
          } catch (error) {
            err = error;
            return reject(err);
          }
      ...
      this.processer.register("categories", (p, posts, ctx) => {
        return new Promise((resolve, reject) => {
          var err, j, len, ref, results, sub;
          try {
            results = [];
            ref = ctx["site"]["categories"];
            for (j = 0, len = ref.length; j < len; j++) {
              sub = ref[j];
              results = results.concat(paginateCategories(sub, p, path.dirname(p["docPath"]), this.site["siteConfig"]["perPage"], ctx));
            }
      ...
      this.processer.register("tags", (p, posts, ctx) => {
        return new Promise((resolve, reject) => {
          var err, j, len, ref, results, sp, tag;
          try {
            results = [];
            ref = ctx["site"]["tags"];
            for (j = 0, len = ref.length; j < len; j++) {
              ...
              sp = Object.assign({}, p);
              sp["layout"] = "tag";
              sp["docPath"] = path.join(path.dirname(sp["docPath"]), `${tag["name"]}`, "index.html");
              tag["docPath"] = sp["docPath"];
              sp["title"] = "tag";
              sp["name"] = tag["name"].toString();
              results = results.concat(paginate(sp, tag["posts"], ctx, this.site["siteConfig"]["perPage"]));
            }
            results.push(Object.assign({}, p, ctx, {
              "tags": ctx["site"]["tags"]
            }));
      ...
      return this.processer.register(["post", "page"], (p, posts, ctx) => {
        return new Promise((resolve, reject) => {
          var $, a, err, getUrl, h, hNames, headings, href, i, imgs, j, l, len, len1, len2, level, links, m, split, src, toc;
          try {
            ...
            for (l = 0, len1 = links.length; l < len1; l++) {
              ...
              $(a).attr("href", path.posix.join(path.posix.sep, path.posix.dirname(p["docPath"]), href));
            }
            imgs = $("img");
            for (m = 0, len2 = imgs.length; m < len2; m++) {
              ...
              $(i).attr("src", path.posix.join(path.posix.sep, path.posix.dirname(p["docPath"]), src));
            }
            p["content"] = $("body").html();
    ...
    registerInternalGenerators() {
      ...
      this.generator.register("beforeProcessing", function(site) {
        // Generate tags.
        return new Promise(function(resolve, reject) {
          var err, found, j, l, len, len1, len2, len3, m, n, newTag, post, postTags, ref, ref1, tag, tagName, tags, tagsLength;
          try {
            tags = [];
            tagsLength = 0;
            ref = site["posts"];
